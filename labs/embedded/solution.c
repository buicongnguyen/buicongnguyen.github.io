...
void DMA_IRQHandler(void) {
    /* Exactly one writer, so a plain load+store replaces an RMW: no LDREX/STREX loop and
     * no libatomic fallback on cores such as Cortex-M0. */
...
int32_t consume(void) {
    ...
    do {
         ...
         * the acquire fence completes every data load before the sequence is re-read. */
