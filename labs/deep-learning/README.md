...
`broken_training.py` trains a binary logistic classifier, but the loss rises. Diagnose it before reading `solution.py`.
...
4. Repair the update rule, rerun the same data, and keep the before/after curves.
5. Add a regression test for loss and accuracy.

The tests import your file by default, so they fail until the repair is right:

```powershell
python -m pytest test_solution.py -q -k "converges or monotonically"   # Part 1: the causal repair
python -m pytest test_solution.py -q                                     # Part 2: also reject invalid configuration
